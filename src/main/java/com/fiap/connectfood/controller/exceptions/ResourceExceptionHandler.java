package com.fiap.connectfood.controller.exceptions;
...
public class ResourceExceptionHandler {
    ...
    @ExceptionHandler(ResourceCnpjNotFoundException.class)
    public ResponseEntity<StandardError> resourceNotFound(ResourceCnpjNotFoundException e, HttpServletRequest request) {
        String error = "Resource not found";
        HttpStatus status = HttpStatus.NOT_FOUND;
        StandardError err = new StandardError(Instant.now(), status.value(), error, e.getMessage(),
                request.getRequestURI());
